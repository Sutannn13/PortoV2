...
export default function LiquidEther({
    ...
    useEffect(() => {
        ...
        class Advection extends ShaderPass {
            ...
            update(...args: any[]) {
                const { dt: d, isBounce: b, BFECC: bf } = (args[0] || {}) as any;
                if (!this.uniforms) return;
                if (typeof d === 'number') this.uniforms.dt.value = d;
                if (typeof b === 'boolean') this.line.visible = b;
        ...
        class ExternalForce extends ShaderPass {
            ...
            update(...args: any[]) {
                ...
                const csX = cs * cellScale.x, csY = cs * cellScale.y;
        ...
        class Viscous extends ShaderPass {
            ...
            update(...args: any[]) {
                const { viscous: v, iterations: it, dt: d } = (args[0] || {}) as any;
                if (!this.uniforms) return;
        ...
        class Divergence extends ShaderPass {
            constructor(simProps: any) { super({ material: { vertexShader: face_vert, fragmentShader: divergence_frag, uniforms: { boundarySpace: { value: simProps.boundarySpace }, velocity: { value: simProps.src.texture }, px: { value: simProps.cellScale }, dt: { value: simProps.dt } } }, output: simProps.dst }); this.init(); }
            update(...args: any[]) { const { vel } = (args[0] || {}) as any; if (this.uniforms && vel) this.uniforms.velocity.value = vel.texture; super.update(); }
        }

        class Poisson extends ShaderPass {
            constructor(simProps: any) { super({ material: { vertexShader: face_vert, fragmentShader: poisson_frag, uniforms: { boundarySpace: { value: simProps.boundarySpace }, pressure: { value: simProps.dst_.texture }, divergence: { value: simProps.src.texture }, px: { value: simProps.cellScale } } }, output: simProps.dst, output0: simProps.dst_, output1: simProps.dst }); this.init(); }
            update(...args: any[]) {
                const { iterations: it } = (args[0] || {}) as any;
        ...
        class Pressure extends ShaderPass {
            constructor(simProps: any) { super({ material: { vertexShader: face_vert, fragmentShader: pressure_frag, uniforms: { boundarySpace: { value: simProps.boundarySpace }, pressure: { value: simProps.src_p.texture }, velocity: { value: simProps.src_v.texture }, px: { value: simProps.cellScale }, dt: { value: simProps.dt } } }, output: simProps.dst }); this.init(); }
            update(...args: any[]) { const { vel, pressure: p } = (args[0] || {}) as any; if (this.uniforms && vel && p) { this.uniforms.velocity.value = vel.texture; this.uniforms.pressure.value = p.texture; } super.update(); }
        }
        ...
        class WebGLManager implements LiquidEtherWebGL {
            ...
            dispose() {
                try {
                    ...
                    Mouse.dispose();
                    if (Common.renderer) { const c = Common.renderer.domElement; if (c && c.parentNode) c.parentNode.removeChild(c); Common.renderer.dispose(); }
                } catch { /* noop */ }
            }
        }
