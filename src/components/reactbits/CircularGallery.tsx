    ...
    createShader() {
      ...
      `,
        ...
        void main() {
          ...
          // Custom card styling to mimic TiltedCard appearance
          // Scale down the icon giving it padding
          float paddingScale = 1.6; 
          vec2 imgUv = (uv - 0.5) * paddingScale + 0.5;

          vec4 texColor = vec4(0.0);
          if (imgUv.x >= 0.0 && imgUv.x <= 1.0 && imgUv.y >= 0.0 && imgUv.y <= 1.0) {
            texColor = texture2D(tMap, imgUv);
          }

          // Card Background Color (#16161e)
          vec3 cardColor = vec3(0.086, 0.086, 0.118);
          // Blend icon over card background
          vec3 mixColor = mix(cardColor, texColor.rgb, texColor.a);
